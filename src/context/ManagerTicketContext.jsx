...
export const ManagerTicketProvider = ({ children }) => {
  ...
  /* ── Report data — prefetched alongside tickets ── */
  ...
  /* ── Single fetch: tickets + stats + reports in parallel ── */
  ...
  /* ── Fetch reports (called on mount + period change) ── */
  ...
  const createTicket = useCallback(async (data) => {
    ...
    /* Optimistic update — show ticket immediately */
    setAllTickets(p => [t, ...p]);
    setStats(p => ({ ...p, total: p.total + 1, open: p.open + 1 }));
    /* Full background refresh — updates tickets list, stats, and reports for all pages */
    fetchAll();
