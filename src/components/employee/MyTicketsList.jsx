          ...
          action={
            !hasActiveFilters && (
              <button onClick={onCreateClick}
                className="px-4 py-2 rounded-xl text-sm font-medium text-white bg-indigo-700 hover:bg-indigo-800 transition-colors">
            ...
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {['Ticket ID', 'Title', 'Category', 'Priority', 'Status', 'SLA', 'Created'].map((h) => (
                  <th key={h} className="text-left px-4 py-3 text-gray-500 font-medium tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {tickets.map((t) => (
                <tr key={t.id} className="hover:bg-indigo-50/30 transition-colors group">
                  <td className="px-4 py-3 font-mono text-indigo-700 font-medium">{t.id}</td>
