...
const SLAChip = ({ slaDueAt, isSlaBreached }) => {
  ...
  return <span className={`text-xs font-semibold ${hrs < 2 ? 'text-orange-600' : 'text-green-600'}`}>
  ...
  return (
    <DashboardLayout title="Overview">
      <div className="space-y-6 animate-fade-in">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Welcome back, {user?.fullName?.split(' ')[0]}</h2>
          ...
          <div className="space-y-2">
            {breached.length > 0 && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 border border-red-200">
                ...
                <p className="text-xs text-red-700 font-medium">{breached.length} ticket{breached.length > 1 ? 's have' : ' has'} breached SLA.</p>
                <button onClick={() => navigate('/dashboard/support/queue')}
                  className="ml-auto text-xs font-semibold text-red-600 hover:underline shrink-0">View queue →</button>
              </div>
            )}
            {urgent.length > 0 && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200">
                <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse shrink-0" />
                <p className="text-xs text-amber-700 font-medium">{urgent.length} ticket{urgent.length > 1 ? 's are' : ' is'} approaching SLA deadline (under 2h).</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm overflow-hidden">
          <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #f3f4f6' }}>
            ...
            <button onClick={() => navigate('/dashboard/support/queue')}
              className="text-xs font-medium hover:underline" style={{ color: '#14a0c8' }}>Open full queue →</button>
          </div>
          {tickets.length === 0 ? (
            <div className="py-12 text-center"><p className="text-sm text-gray-400">Your queue is empty. 🎉</p></div>
          ) : (
