...
const SupportQueue = () => {
  ...
  const { tickets, updateStatus, addComment } = useTickets(user?.id, 'SUPPORT_STAFF');
    ...
    <DashboardLayout title="My Queue">
      <div className="space-y-5 animate-fade-in">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Select a ticket to update its status or add a comment.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          <div className="lg:col-span-2">
            <AssignedTicketQueue tickets={tickets} onSelectTicket={setSelected} selectedId={selected?.id} />
          </div>
              ...
              <>
                <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm p-5">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                      <p className="font-mono text-xs font-medium mb-1" style={{ color: '#3c3c8c' }}>{selected.id}</p>
                ...
                <CommentAttachmentPanel ticket={selected} onAddComment={handleAddComment} authorName={user?.fullName} />
              </>
            ) : (
              <div className="h-72 flex items-center justify-center bg-white rounded-2xl border border-dashed border-gray-200">
                <div className="text-center">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"
                    strokeLinecap="round" strokeLinejoin="round" className="w-8 h-8 text-gray-300 mx-auto mb-3">
